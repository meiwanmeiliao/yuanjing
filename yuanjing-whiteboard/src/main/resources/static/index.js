...
function renderAllPaths() {
    ...
    drawingCxt.stroke();
    console.log(paths);
	console.log(mouse_move_all);
	console.log(draw_mouse_move_all);
}
...
function drawEnd() {
	...
	mouse_move_all.push(mouse_move);
	mouse_move = [];
	draw_mouse_move_all.push(draw_mouse_move);
	draw_mouse_move = [];
    currentPath = [];
...
function drawMove(e) {
	mouse_move.push([Date.now(), e.pageX,e.pageY]);
    if(drawingDisabled) return;
	...
	console.log(Date.now()+":"+ e.pageX+":"+e.pageY);
    currentPath.push([e.pageX, e.pageY]);
	draw_mouse_move.push([Date.now(), e.pageX,e.pageY]);
